...
function angrySun.onTickEndNPC(v)
	...
	if data.state == 0 then -- First spawned in
		data.lerpGoalPos = vector(cam.x + 0.5 * cam.width + 0.4 * cam.width * v.direction, cam.y + cam.height * 0.5 - cfg.cameraPositionY * cam.height)
		data.state = -1
	...
	elseif data.state == 1 then -- Rest in the corner of the screen
		...
		if v.y + 0.5 * v.height ~= cam.y + cam.height * 0.5 - cfg.cameraPositionY * cam.height then
			v.y = math.lerp(v.y + 0.5 * v.height, cam.y + cam.height * 0.5 - cfg.cameraPositionY * cam.height, data.timer/140) - v.height * 0.5
		end
	...
	elseif data.state == 2 then -- Preparing to swoop down
		...
		if data.lerpStartPos.y ~= cam.y + cam.height * 0.5 - cfg.cameraPositionY * cam.height then
			v.y = math.lerp(data.lerpStartPos.y, cam.y + cam.height * 0.5 - cfg.cameraPositionY * cam.height, data.timer/140)
		end
		data.timer = data.timer + 1
	...
	elseif data.state == 3 then -- Swoop down
		...
		if data.lerp <= 50 then
			...
			if data.lerp + 0.75 >= 50 then
				...
				data.lerpGoalPos.y = cam.y + cam.height * 0.5 - cfg.cameraPositionY * cam.height
			end
		else
