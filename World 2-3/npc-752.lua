...
local function init(v,data,settings)
	...
	if not settings.angle then
		...
		settings.appearRateMult = 1
	end

	data.currentCollider = Colliders.Rect(0, 0, settings.width, settings.limit*32)
	data.blockChecker = Colliders.Box(0, 0, 0, 0)

	if settings.autoCalculateMultiplier then

		settings.appearRateMult = settings.width / settings.autoCalculateWidth

	end

	data.particleEffect = Particles.Emitter(0, 0, Misc.resolveFile("bubble_particle.ini"))
	data.particleEffect:setParam("limit", data.particleEffect:getParamDefault("limit") * settings.appearRateMult)
...
function sampleNPC.onDrawNPC(v)
	...
	data.particleEffect:setParam("rate", spd*9*settings.appearRateMult)
