...
local audioeffect = Misc.resolveSoundFile("hammerFanfareLooped")
...
function hammer.onStart()
	upHammer_collision = Colliders.Box(player.x + player.width * 0.5 - 16, player.y - configFile.height - 2, 32, 32)
...
function hammer.onTick()
	...
	if hammerTimer > 0 then
		...
		if configFile.usesfx and not hammerSoundObject then
			musicCutOff = Audio.MusicGetPos()

			Routine.run(function()
				Routine.skip() 

				Audio.SeizeStream(-1)
				hammerSoundObject = true
				Audio.MusicOpen(audioeffect)
				Audio.MusicPlay()
			end)
		end
	else
...
function hammer.onDraw()
	if upHammer_collision ~= nil then
		upHammer_collision.x = player.x + player.width * 0.5 - 16
		upHammer_collision.y = player.y - configFile.height - 2
	end
	
	if player.direction == DIR_LEFT then
		if sideHammer_collision ~= nil then
			sideHammer_collision.x = player.x - 34
			player_sy = 1
		end
	else
		if sideHammer_collision ~= nil then
			sideHammer_collision.x = player.x + player.width + 2
			player_sy = 2
		end
	end

	if sideHammer_collision ~= nil then
		sideHammer_collision.y = player.y + player.height - 34
	end
