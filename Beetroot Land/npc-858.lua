...
function beetroot.onInitAPI()
	npcManager.registerEvent(npcID, beetroot, "onStartNPC")
	npcManager.registerEvent(npcID, beetroot, "onTickNPC")
	registerEvent(beetroot, "onNPCHarm")
end

function beetroot.onStartNPC(v)
	for i,p in ipairs(NPC.get(610)) do
		p.collisionGroup = "Lakitus"
	end
	Misc.groupsCollide["beetroot"]["beetroot"] = false
	Misc.groupsCollide["beetroot"]["Lakitus"] = false
end
...
function beetroot.onTickNPC(v)
	...
	if not data.initialized then
		...
		if v.speedX < 0 then
		v.speedX = v.speedX * (v.direction * -1)
		end
	end
...
function beetroot.onNPCHarm(token,v,harm,c)
	if type(c) == "NPC" and c.id == npcID and v.id == npcID-1 then token.cancelled = true return end
	if not c or type(c) ~= "NPC" or c.id ~= npcID or v.id == npcID+1 then return end
