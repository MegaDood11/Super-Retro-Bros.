...
function CheckboardGrey.onTickNPC(v)
	...
	if data._settings.direction == 0 then
		v.speedX = math.sin((-1* -v.direction)*data.movespeed)*NPC.config[npcID].speed*1.375
	elseif data._settings.direction == 1 then
		v.speedY = math.sin((-1* -v.direction)*data.movespeed)*NPC.config[npcID].speed*1.375
	end
end
