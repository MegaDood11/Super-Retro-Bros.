...
local default = Defines.player_runspeed
...
function block.onTickBlock(v)
	...
	Defines.player_runspeed = default
end
...
function block.onCollideBlock(v,n)
	...
	if (n.__type == "Player") then
		...
		if v.data._settings.force >= Defines.player_runspeed then Defines.player_runspeed = v.data._settings.force end
