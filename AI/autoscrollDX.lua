...
--Edited by DeviousQuacks23, 2025 (happy 10th anniversary?)
...
function autoscroll.onTick()
	...
	screenWidth, screenHeight = Graphics.getMainFramebufferSize()
...
function autoscroll.lockScreen(playerIdx)
	...
	local x2, y2 = x1 + (Camera.get()[playerIdx].width), y1 + (Camera.get()[playerIdx].height) -- Incompatible with split-screen
	setSectionBounds(section, x1, y1, y2, x2)
	active_scrolls[section] = nil
end
...
function autoscroll.scrollTo(left, bottom, speed, section)
	...
	local x2 = x1 + screenWidth
	local y1 = y2 - screenHeight
