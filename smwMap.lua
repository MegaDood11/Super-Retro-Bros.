...
do
    ...
    local function tryMove(v,directionName)
        ...
        if player.keys[directionName] ~= KEYS_DOWN then
            return false
        end
    ...
    local function updateActiveAreas(v,padding)
        ...
        for _,areaObj in ipairs(smwMap.areas) do
            ...
            if areaObj.collider:collide(collider) then
				...
				local worldNames = Graphics.loadImageResolved("smwMap/worldNames.png")
			
				worldNameList = worldNameList or Sprite{texture = worldNames, frames = 14}
				worldNameListID = areaObj.worldName
				worldNameList.position = vector(smwMap.hudSettings.borderLeftWidth + 300, smwMap.hudSettings.borderTopHeight - player.height + smwMap.hudSettings.playerOffsetY + 34)
				worldNameList:draw{sceneCoords = false, frame = worldNameListID, priority = 5}
...
do
    ...
    function smwMap.drawScenery(v)
        ...
        basicGlDrawArgs.priority = config.priority or getSceneOrPlayerPriority(v.x,v.y + v.height*0.5)
    ...
    function smwMap.drawPath(pathObj)
        ...
        for _,drawGroup in ipairs(pathObj.drawGroups) do
            ...
            for i = 1, groupVertexCount, 2 do
                vc[i  ] = groupVC[i  ] - smwMap.camera.x
                vc[i+1] = groupVC[i+1] - smwMap.camera.y
    ...
    smwMap.hudCounters = {
        ...
        -- Coins
        {
            icon = Graphics.loadImageResolved("smwMap/hud_coin.png"),
            getValue = (function()
        ...
        if hudSettings.countersEnabled then
            ...
            for _,counter in ipairs(smwMap.hudCounters) do
                ...
                if (counter.isEnabled == nil or counter.isEnabled()) and icon ~= nil then
                    widestIconWidth = math.max(widestIconWidth,icon.width + 16)
                end
            end
...
smwMap.hudSettings = {
    ...
    levelTitleFont = textplus.loadFont("smb1HUD/smb1-font.ini"),
    levelTitleColor = Color.white,
    levelTitleOffsetX = 32,
    levelTitleOffsetY = -20,
    levelTitleScale = 2,
